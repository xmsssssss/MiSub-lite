...
const needsSetup = computed(() => sessionStore.publicConfig?.needsSetup === true && !isLoggedIn.value);
...
const loginComponent = computed(() => {
  if (needsSetup.value) return SetupWizard;
...
async function handleSetupSuccess(payload = {}) {
  if (sessionStore.publicConfig) {
    sessionStore.publicConfig = { ...sessionStore.publicConfig, needsSetup: false };
  }
  if (payload.password) {
    try {
...
watch(
  ...
  () => {
    ...
    document.title = shouldHidePublicBranding.value
      ...
      : (rawTitle ? `${rawTitle} - MiSub-lite` : 'MiSub-lite');
  },
  { immediate: true }
);
...
<button @click="showUpdateNotice = false" class="rounded-md px-2 py-1 text-sm hover:bg-amber-100/80 dark:hover:bg-white/10">{{ t('actions.gotIt') }}</button>
      ...
      <template v-else-if="needsSetup">
        <SetupWizard @success="handleSetupSuccess" />
      </template>
      ...
      <template v-else>
        <component :is="loginComponent" :login="login" @success="handleSetupSuccess" />
      </template>
