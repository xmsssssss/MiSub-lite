 ...
 * 首次引导：仅在仍使用默认密码时可用
 */
export async function handleSetupRequest(request, env) {
    ...
    try {
        if (!(await isUsingDefaultPassword(env))) {
            return createErrorResponse('系统已完成初始化，请直接登录', 403);
        }
        ...
        if (password === 'admin') {
            return createErrorResponse('请不要继续使用默认密码 admin', 400);
        }
        ...
        if (body?.cookieSecret && String(body.cookieSecret).trim()) {
            const secret = String(body.cookieSecret).trim();
        ...
        // Optional first-run app settings
        const settingsPatch = {};
        if (body?.siteName || body?.FileName) {
            settingsPatch.FileName = String(body.siteName || body.FileName).trim();
        }
        if (body?.subscriptionToken || body?.mytoken) {
            settingsPatch.mytoken = String(body.subscriptionToken || body.mytoken).trim();
        }
        if (body?.profileToken) {
            settingsPatch.profileToken = String(body.profileToken).trim();
        }
        if (body?.customLoginPath) {
            settingsPatch.customLoginPath = String(body.customLoginPath).trim().replace(/^\/+/, '');
        }
        if (typeof body?.enablePublicPage === 'boolean') {
            settingsPatch.enablePublicPage = body.enablePublicPage;
        }
        ...
        // Mirror non-secret options into config.yaml when available
        if (typeof env.persistLocalConfig === 'function') {
            const filePatch = { adminPassword: password };
            if (body?.publicUrl) filePatch.publicUrl = String(body.publicUrl).trim();
            if (body?.callbackUrl) filePatch.callbackUrl = String(body.callbackUrl).trim();
            ...
            await env.persistLocalConfig(filePatch);
            if (filePatch.publicUrl) {
                env.MISUB_PUBLIC_URL = filePatch.publicUrl;
                if (typeof process !== 'undefined' && process.env) {
                    process.env.MISUB_PUBLIC_URL = filePatch.publicUrl;
                }
            }
        }
        ...
        return createErrorResponse(e?.message || '初始化失败', 500);
    }
}
