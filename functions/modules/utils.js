...
function getRuntimeEnvValue(env, key) {
    ...
    // Node 本地：env 对象未注入时回退 process.env
    if (typeof process !== 'undefined' && process.env) {
        const procValue = process.env[key];
        if (procValue !== undefined && procValue !== null && String(procValue).trim() !== '') {
            return String(procValue);
        }
    }
...
/**
 * 本地 config.yaml 中的管理员密码（每次读取，改配置后无需重启即可登录）。
 */
async function getLocalConfigAdminPassword(env) {
    if (env?.MISUB_RUNTIME !== 'node-local') return null;
    if (typeof env?.readLocalAdminPassword === 'function') {
        try {
            const value = await env.readLocalAdminPassword();
            if (value && String(value).trim()) return String(value).trim();
        } catch (error) {
            console.warn('[Auth] readLocalAdminPassword failed:', error?.message || error);
        }
    }
    return null;
}
...
async function getSystemSettingValue(env, key) {
    ...
    try {
        ...
        console.warn(`[Auth Storage] DB get skipped for ${key}:`, error?.message || error);
        return null;
    }
}
...
/**
 * 获取或生成 Cookie 加密密钥
 * 优先顺序：KV → SQLite/D1 → 环境变量 COOKIE_SECRET → 随机生成
...
export async function getCookieSecret(env) {
    ...
    if (kv) {
        const kvSecret = await safeKvGet(kv, SYSTEM_COOKIE_SECRET_KEY);
        if (kvSecret) return kvSecret;

        if (runtimeCookieSecret) {
            await safeKvPut(kv, SYSTEM_COOKIE_SECRET_KEY, runtimeCookieSecret);
            return runtimeCookieSecret;
        }

        const newSecret = crypto.randomUUID();
        await safeKvPut(kv, SYSTEM_COOKIE_SECRET_KEY, newSecret);
        return newSecret;
    }

    const dbSecret = await getSystemSettingValue(env, SYSTEM_COOKIE_SECRET_KEY);
    if (dbSecret) return String(dbSecret).trim();

    if (runtimeCookieSecret) {
        await putSystemSettingValue(env, SYSTEM_COOKIE_SECRET_KEY, runtimeCookieSecret);
        return runtimeCookieSecret;
    }

    const generated = crypto.randomUUID();
    await putSystemSettingValue(env, SYSTEM_COOKIE_SECRET_KEY, generated);
    return generated;
}
 ...
 * 获取管理员密码
 * 优先顺序：
 * 1) 系统环境变量 ADMIN_PASSWORD（非 config 注入的显式 env）
 * 2) 本地 config.yaml（每次实时读取，支持 adminPassword / password 等别名）
 * 3) env.ADMIN_PASSWORD（启动时注入）
 * 4) KV → SQLite → 默认 admin
 * @param {Object} env - 运行时环境对象
 * @returns {Promise<string>} 密码
 */
export async function getAdminPassword(env) {
    // 显式系统环境变量（用户 export/set 的）优先于文件
    if (typeof process !== 'undefined' && process.env?.ADMIN_PASSWORD_FROM_SYSTEM === '1') {
        const systemPwd = process.env.ADMIN_PASSWORD;
        if (systemPwd && String(systemPwd).trim()) return String(systemPwd).trim();
    }

    const localConfigPassword = await getLocalConfigAdminPassword(env);
    if (localConfigPassword) {
        return localConfigPassword;
    }
    ...
    const kv = getKV(env);
    if (kv) {
        const kvPassword = await safeKvGet(kv, SYSTEM_ADMIN_PASSWORD_KEY);
        if (kvPassword) return String(kvPassword).trim();
    }

    const dbPassword = await getSystemSettingValue(env, SYSTEM_ADMIN_PASSWORD_KEY);
...
/**
 * 设置管理员密码
 * 优先写 SQLite/D1，其次 KV；本地可同步写 config.yaml
 * @param {Object} env - 运行时环境对象
...
export async function setAdminPassword(env, newPassword) {
    ...
    if (env?.MISUB_DB) {
        persisted = await putSystemSettingValue(env, SYSTEM_ADMIN_PASSWORD_KEY, password);
    }

    const kv = getKV(env);
    if (kv) {
        const ok = await safeKvPut(kv, SYSTEM_ADMIN_PASSWORD_KEY, password);
 ...
 * Kept local so backend does not depend on src/ in production Docker images.
 */
